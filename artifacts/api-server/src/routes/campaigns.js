...
import { CreateCampaignBody, GetCampaignParams, UpdateCampaignParams, UpdateCampaignBody, DeleteCampaignParams } from "@workspace/api-zod";
import { requireAuth } from "../middlewares/requireAuth";
const MAX_IMAGE_DATA_URL = 2_000_000; // ~2MB base64 cap per image field

function validateIdList(raw, field) {
  ...
  if (typeof raw !== "string") return {
    ok: false,
    error: `${field} deve ser uma string JSON`
  };
  try {
    ...
    if (!arr.every(x => typeof x === "number" && Number.isInteger(x) && x > 0)) {
      return {
        ok: false,
        error: `${field} deve conter apenas IDs inteiros positivos`
      };
    }
    return {
      ok: true,
    ...
    return {
      ok: false,
      error: `${field} contém JSON inválido`
    };
  }
}
function validateImage(raw, field) {
  ...
  if (raw.length > MAX_IMAGE_DATA_URL) return {
    ok: false,
    error: `${field} excede tamanho máximo (~2MB)`
  };
  return {
    ok: true
  };
...
router.get("/campaigns", requireAuth, async (req, res) => {
  ...
  const rows = await db.select().from(campaignsTable).where(eq(campaignsTable.userId, userId)).orderBy(campaignsTable.createdAt);
...
router.get("/campaigns/:id", requireAuth, async (req, res) => {
  ...
  const [row] = await db.select().from(campaignsTable).where(and(eq(campaignsTable.id, params.data.id), eq(campaignsTable.userId, userId)));
  if (!row) {
    res.status(404).json({
...
router.patch("/campaigns/:id", requireAuth, async (req, res) => {
  ...
  const updateData = {};
  const passthrough = ["name", "coverUrl", "synopsis", "setting", "status", "currentArc", "nextSession", "partyName", "playerCharacterIds", "bossIds", "sessionLog", "npcs", "locations", "notes"];
  for (const k of passthrough) {
    if (d[k] !== undefined) updateData[k] = d[k];
  }
  const [row] = await db.update(campaignsTable).set(updateData).where(and(eq(campaignsTable.id, params.data.id), eq(campaignsTable.userId, userId))).returning();
  if (!row) {
    res.status(404).json({
      error: "Campanha não encontrada"
    });
...
router.delete("/campaigns/:id", requireAuth, async (req, res) => {
  ...
  const deleted = await db.delete(campaignsTable).where(and(eq(campaignsTable.id, params.data.id), eq(campaignsTable.userId, userId))).returning({

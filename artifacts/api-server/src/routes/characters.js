...
import { ListCharactersQueryParams, CreateCharacterBody, GetCharacterParams, UpdateCharacterParams, UpdateCharacterBody, DeleteCharacterParams } from "@workspace/api-zod";
...
router.get("/characters", requireAuth, async (req, res) => {
  ...
  const characters = await db.select().from(charactersTable).where(and(...conditions)).orderBy(charactersTable.createdAt);
...
router.get("/characters/:id", requireAuth, async (req, res) => {
  ...
  const [character] = await db.select().from(charactersTable).where(and(eq(charactersTable.id, params.data.id), eq(charactersTable.userId, userId)));
  if (!character) {
    res.status(404).json({
...
router.patch("/characters/:id", requireAuth, async (req, res) => {
  const userId = req.userId;
  ...
  const [character] = await db.update(charactersTable).set(updateData).where(and(eq(charactersTable.id, params.data.id), eq(charactersTable.userId, userId))).returning();
  if (!character) {
    res.status(404).json({
      error: "Personagem não encontrado"
    });
...
router.delete("/characters/:id", requireAuth, async (req, res) => {
  ...
  const [character] = await db.delete(charactersTable).where(and(eq(charactersTable.id, params.data.id), eq(charactersTable.userId, userId))).returning();
  if (!character) {
    res.status(404).json({
      error: "Personagem não encontrado"
    });
...
function mapCharacterSummary(c) {
  return {
    id: c.id,
    name: c.name,
    level: c.level,

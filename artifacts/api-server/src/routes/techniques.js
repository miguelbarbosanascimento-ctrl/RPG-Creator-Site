...
import { ListTechniquesQueryParams, CreateTechniqueBody, GetTechniqueParams } from "@workspace/api-zod";
const router = Router();
router.get("/techniques", async (req, res) => {
  ...
  const techniques = conditions.length > 0 ? await db.select().from(techniquesTable).where(and(...conditions)).orderBy(techniquesTable.name) : await db.select().from(techniquesTable).orderBy(techniquesTable.name);

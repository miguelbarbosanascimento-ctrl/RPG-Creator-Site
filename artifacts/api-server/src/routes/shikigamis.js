...
router.get("/shikigamis", requireAuth, async (req, res) => {
...
router.post("/shikigamis", requireAuth, async (req, res) => {
  const userId = req.userId;
...
router.delete("/shikigamis/:id", requireAuth, async (req, res) => {
  const userId = req.userId;
  ...
  const deleted = await db.delete(shikigamisTable).where(and(eq(shikigamisTable.id, params.data.id), eq(shikigamisTable.userId, userId))).returning({
    id: shikigamisTable.id
  });
  if (deleted.length === 0) {
    res.status(404).json({
      error: "Shikigami não encontrado"
    });
    return;
  }
  res.status(204).send();

...
import { CreateBossBody, GetBossParams, UpdateBossParams, UpdateBossBody, DeleteBossParams } from "@workspace/api-zod";
...
router.get("/bosses/:id", requireAuth, async (req, res) => {
  const userId = req.userId;
  ...
  const [row] = await db.select().from(bossesTable).where(and(eq(bossesTable.id, params.data.id), eq(bossesTable.userId, userId)));
  if (!row) {
    res.status(404).json({
...
router.patch("/bosses/:id", requireAuth, async (req, res) => {
  const userId = req.userId;
  ...
  const updateData = {};
  const passthrough = ["name", "photoUrl", "vd", "category", "size", "grade", "domain", "description", "appearance", "strength", "dexterity", "constitution", "intelligence", "wisdom", "charisma", "hp", "maxHp", "energy", "maxEnergy", "armorClass", "attention", "movement", "hitDice", "abilities", "innateTechnique", "techniqueDescription", "weaknesses", "resistances", "loot", "notes"];
  for (const k of passthrough) {
    if (d[k] !== undefined) updateData[k] = d[k];
  }
  const [row] = await db.update(bossesTable).set(updateData).where(and(eq(bossesTable.id, params.data.id), eq(bossesTable.userId, userId))).returning();
  if (!row) {
    res.status(404).json({
      error: "Boss não encontrado"
    });
...
router.delete("/bosses/:id", requireAuth, async (req, res) => {
  ...
  const deleted = await db.delete(bossesTable).where(and(eq(bossesTable.id, params.data.id), eq(bossesTable.userId, userId))).returning({
    id: bossesTable.id
  });
  if (deleted.length === 0) {
    res.status(404).json({
